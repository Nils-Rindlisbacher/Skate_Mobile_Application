...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    // Optimized bulk fetch to solve N+1 problem
    ...
    @Query(value = "SELECT u.id as id, u.name as name, u.username as username, " +
            ...
            " AND (:stance IS NULL OR ct.stance = :stance)) as completedCount " +
            "FROM users u " +
            "ORDER BY completedCount DESC", nativeQuery = true)
    List<LeaderboardProjection> getLeaderboardData(@Param("category_id") Long categoryId, @Param("stance") String stance);
}
